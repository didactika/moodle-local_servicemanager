...
/**
 * Unit tests for updating a schema: the version rules, and repairing Moodle
 * objects that were deleted behind the plugin's back.
 *
 ...
 * @covers     \local_servicemanager\schema\provisioner
 * @covers     \local_servicemanager\schema\version_policy
 */
final class manager_update_test extends \advanced_testcase {
    /**
...
YAML;
    ...
    /**
     * Test that restore_schema accepts a version an ordinary update would reject.
     */
    public function test_restore_schema_allows_going_backwards(): void {
        global $DB;
        $this->resetAfterTest();
        $this->setAdminUser();
...
YAML;
        ...
        // Restoring the original content moves the version back down.
        $manager->restore_schema($id, $this->get_valid_yaml());

        $schema = $DB->get_record('local_servicemanager_schemas', ['id' => $id]);
        $this->assertEquals('1.0.0', $schema->version);
    }
